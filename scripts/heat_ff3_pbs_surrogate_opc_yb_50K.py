...
OUTPUT_CHECKPOINT = Path(
    "checkpoints/"
    "ff3_pbs_surrogate_opc_yb_heating_050K.chk"
)

OUTPUT_LOG = Path(
    "analysis/"
    "ff3_pbs_surrogate_opc_yb_heating_050K_log.csv"
)
...
if metadata_graphene_particles != N_GRAPHENE_TOTAL:
    ...
    raise RuntimeError(
        "Graphene particle count disagrees with "
        "assembly metadata."
    )
...
if metadata_ligand_particles != N_LIGAND:
    ...
    raise RuntimeError(
        "Ligand particle count disagrees with "
        "assembly metadata."
    )
...
if metadata_sites_per_water != SITES_PER_WATER:
    ...
    raise RuntimeError(
        "OPC sites-per-water disagrees with "
        "assembly metadata."
    )
...
if cl_start_index != na_end_index:
    ...
    raise RuntimeError(
        "Na+/Cl- particle boundary mismatch."
    )
...
if cl_end_index != metadata_total_particles:
    ...
    raise RuntimeError(
        "Final particle-range endpoint disagrees "
...
if n_particles != metadata_total_particles:
    ...
    raise RuntimeError(
        "System particle count disagrees with "
        "assembly metadata. "
