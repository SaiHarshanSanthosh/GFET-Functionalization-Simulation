...
OUTPUT_POSITIONS = Path(
    "parameters/combined/"
    "ff3_pbs_surrogate_opc_yb_minimized_positions_nm.npy"
)

OUTPUT_METADATA = Path(
    "analysis/"
    "ff3_pbs_surrogate_opc_yb_minimization.json"
)
...
# PBS-surrogate target-system bookkeeping.
N_RETAINED_WATERS = 6065
...
NA_START_INDEX = WATER_END_INDEX
NA_END_INDEX = (
    NA_START_INDEX
    + N_NA
)

CL_START_INDEX = NA_END_INDEX
CL_END_INDEX = (
    CL_START_INDEX
    + N_CL
)

N_TOTAL_EXPECTED = CL_END_INDEX

if WATER_END_INDEX != 28080:
    raise RuntimeError(
        "Unexpected retained-water end index."
    )

if NA_START_INDEX != 28080 or NA_END_INDEX != 28098:
    raise RuntimeError(
        "Unexpected Na particle-index range."
    )

if CL_START_INDEX != 28098 or CL_END_INDEX != 28116:
    raise RuntimeError(
        "Unexpected Cl particle-index range."
    )

if N_TOTAL_EXPECTED != 28116:
    raise RuntimeError(
        "Unexpected PBS-surrogate total particle count."
    )
...
if n_particles != N_TOTAL_EXPECTED:
    ...
    raise RuntimeError(
        "PBS-surrogate particle count mismatch. "
...
# ============================================================
# 7. VERIFY PBS-SURROGATE COUNT CONSISTENCY
# ============================================================
