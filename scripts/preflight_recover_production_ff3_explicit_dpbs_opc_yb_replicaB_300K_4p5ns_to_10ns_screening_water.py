...
POSITIONS_FILE = Path(
    ...
    "equilibrated_300K_1ns_positions_nm.npy"
)

VELOCITIES_FILE = Path(
    "parameters/combined/"
    "ff3_explicit_dpbs_opc_yb_replicaB_"
    "equilibrated_300K_1ns_velocities_nm_per_ps.npy"
)
...
PRIOR_STAGE_METADATA = Path(
    "analysis/"
    "ff3_explicit_dpbs_opc_yb_replicaB_equilibration_300K_1ns.json"
)

INPUT_CHECKPOINT = Path(
    "checkpoints/"
    "ff3_explicit_dpbs_opc_yb_replicaB_"
    "production_300K_10ns_rescue.chk"
)
...
OUTPUT_CHECKPOINT = Path(
    "checkpoints/"
    "ff3_explicit_dpbs_opc_yb_replicaB_production_300K_10ns.chk"
)

OUTPUT_LOG = Path(
    "analysis/"
    "ff3_explicit_dpbs_opc_yb_replicaB_production_300K_10ns_log.csv"
)

OUTPUT_METADATA = Path(
    "analysis/"
    "ff3_explicit_dpbs_opc_yb_replicaB_production_300K_10ns.json"
)

SCREENING_TRAJECTORY = Path(
    "analysis/"
    "ff3_explicit_dpbs_opc_yb_replicaB_production_300K_10ns_"
...
WATER_TRAJECTORY = Path(
    "analysis/"
    "ff3_explicit_dpbs_opc_yb_replicaB_production_300K_10ns_"
    "opc_ohh_10ps_float32.npy"
)
...
RESCUE_CHECKPOINT = Path(
    "checkpoints/"
    "ff3_explicit_dpbs_opc_yb_replicaB_production_300K_10ns_rescue.chk"
)
...
WATER_TRAJECTORY_INTERVAL_PS = 10.0

WATER_TRAJECTORY_STEPS = int(
    round(
        WATER_TRAJECTORY_INTERVAL_PS
        /
        TIMESTEP_PS
    )
)
...
if (
    ...
    raise RuntimeError(
        "Water trajectory interval must be divisible "
        "by the 1 ps reporting interval."
    )
...
if (
    ...
    raise RuntimeError(
        ...
        "by the water trajectory interval."
    )
...
for output_path in [
    ...
    if output_path.exists():
        ...
        raise RuntimeError(
            "Refusing to overwrite existing final output: "
            f"{output_path}"
        )
...
for recovery_path in [
    ...
    if not recovery_path.exists():
        ...
        raise FileNotFoundError(
            "Required interrupted-run recovery file "
            f"not found: {recovery_path}"
        )
...
with open(
    ...
    rescue_progress = json.load(f)


if rescue_progress.get(
    "status"
) != "PRODUCTION_RESCUE_CHECKPOINT":

    raise RuntimeError(
        "Recovery requires a valid "
        "PRODUCTION_RESCUE_CHECKPOINT progress file. "
        f"Found: {rescue_progress.get('status')}"
    )


RECOVERY_START_PRODUCTION_PS = float(
    rescue_progress[
        "production_elapsed_ps"
    ]
)

RESCUE_CONTEXT_TIME_PS = float(
    rescue_progress[
        "openmm_context_time_ps"
    ]
)

RESCUE_SCREENING_FRAMES = int(
    rescue_progress[
        "screening_frames_written"
    ]
)

RESCUE_WATER_FRAMES = int(
    rescue_progress[
        "water_frames_written"
    ]
)


RECOVERY_REMAINING_PS = (
    TOTAL_TIME_PS
    -
    RECOVERY_START_PRODUCTION_PS
)

if RECOVERY_REMAINING_PS <= 0.0:

    raise RuntimeError(
        "No production time remains to recover."
    )
...
if (
    RESCUE_SCREENING_FRAMES
    !=
    expected_screening_frames
):

    raise RuntimeError(
        "Rescue screening-frame count disagrees "
        "with elapsed production time."
    )
...
if (
    ...
    raise RuntimeError(
        "Rescue water-frame count disagrees "
        "with elapsed production time."
    )
...
if (
    ...
    raise RuntimeError(
        "Recovery point is not on a rescue-checkpoint boundary."
    )
...
if (
    ...
    raise RuntimeError(
        "Recovery point is not on a water-frame boundary."
    )


if prior_stage_metadata.get(
    "status"
) != "EQUILIBRATION_300K_1NS_PASS":

    raise RuntimeError(
        "Replica B production requires a successful "
        "EQUILIBRATION_300K_1NS_PASS input stage. "
...
# ============================================================
# ============================================================
# 12. LOAD EXACT PRODUCTION RESCUE CHECKPOINT
#
# This checkpoint was atomically written at the last completed
# 100 ps production boundary before interruption.
#
# NO setPositions()
# NO setVelocities()
# NO setTime()
# NO applyConstraints()
# NO applyVelocityConstraints()
# NO computeVirtualSites()
# NO velocity regeneration
# NO minimization
# ============================================================

checkpoint_bytes = (
    INPUT_CHECKPOINT.read_bytes()
)
...
# ============================================================
# 13. VERIFY EXACT RESCUE CHECKPOINT CONTINUITY
# ============================================================
...
if not np.isfinite(
    ...
    raise RuntimeError(
        "Recovery checkpoint time is non-finite."
    )


checkpoint_time_error_ps = abs(
    checkpoint_time_ps
    -
    RESCUE_CONTEXT_TIME_PS
)


if checkpoint_time_error_ps > 1.0e-6:

    raise RuntimeError(
        "Recovery checkpoint clock disagrees "
        "with rescue progress JSON: "
        f"{checkpoint_time_error_ps:.9e} ps"
    )


scientific_equilibration_offset_ps = (
    checkpoint_time_ps
    -
    RECOVERY_START_PRODUCTION_PS
)
...
if abs(
    ...
    raise RuntimeError(
        "Checkpoint clock / production elapsed relationship "
        "does not preserve the expected 1000 ps "
        "equilibration prefix."
    )


print()
print("=" * 78)
print("EXACT PRODUCTION RESCUE CHECKPOINT")
print("=" * 78)

print(
    f"Production already completed: "
    f"{RECOVERY_START_PRODUCTION_PS:.3f} ps"
)

print(
    f"Production remaining: "
    f"{RECOVERY_REMAINING_PS:.3f} ps"
)

print(
    f"Checkpoint OpenMM time: "
    f"{checkpoint_time_ps:.12f} ps"
)

print(
    f"Rescue JSON OpenMM time: "
    f"{RESCUE_CONTEXT_TIME_PS:.12f} ps"
)

print(
    f"Checkpoint/JSON time difference: "
    f"{checkpoint_time_error_ps:.3e} ps"
)

print(
    f"Persisted screening frames: "
    f"{RESCUE_SCREENING_FRAMES}"
)

print(
    f"Persisted water frames: "
    f"{RESCUE_WATER_FRAMES}"
)

print("Position reset: NONE")
print("Velocity reset/regeneration: NONE")
print("Time reset: NONE")
print("Constraint reprojection: NONE")
...
# ============================================================
# 14A. OPC WATER STORAGE / RECONSTRUCTION AUDIT
#
# Stored trajectory:
#     O, H1, H2 only
#
# Reconstructed site:
#     M = wO*O + wH1*H1 + wH2*H2
#
# The virtual-site rule is read directly from the serialized
# Replica B Hamiltonian and verified for every OPC molecule.
# ============================================================

water_first_index = water_start_index
...
if not system.isVirtualSite(
    first_m_index
):
    raise RuntimeError(
        "First OPC M site is not virtual."
    )

first_m_site = system.getVirtualSite(
    first_m_index
)
...
if not isinstance(
    ...
    raise RuntimeError(
        "Unexpected OPC M-site virtual-site type."
    )

OPC_M_WEIGHTS = (
    float(first_m_site.getWeight(0)),
    float(first_m_site.getWeight(1)),
    float(first_m_site.getWeight(2)),
)

for water_i in range(
    n_waters
):
    base = (
        water_start_index
    ...
    if (
        system.isVirtualSite(base)
        or
        system.isVirtualSite(base + 1)
        or
        system.isVirtualSite(base + 2)
        or
        not system.isVirtualSite(base + 3)
    ):
        raise RuntimeError(
            "OPC O-H-H-M ordering audit failed "
            f"at water {water_i}."
        )
    ...
    parents = (
        int(vs.getParticle(0)),
        int(vs.getParticle(1)),
        int(vs.getParticle(2)),
    )
    ...
    if parents != (
        ...
        raise RuntimeError(
            "OPC M-site parent audit failed "
            f"at water {water_i}."
        )

    weights = (
        float(vs.getWeight(0)),
        float(vs.getWeight(1)),
        float(vs.getWeight(2)),
    )

    if weights != OPC_M_WEIGHTS:
        raise RuntimeError(
            "Non-uniform OPC M-site reconstruction "
            f"at water {water_i}."
        )


water_nonbonded_force = None

for force_i in range(
    system.getNumForces()
):
    force = system.getForce(
        force_i
    ...
    if isinstance(
        ...
        water_nonbonded_force = force
        break

if water_nonbonded_force is None:
    raise RuntimeError(
        "NonbondedForce not found for OPC charge audit."
    )
...
for local_i in range(4):
    ...
    OPC_SITE_CHARGES_E.append(
        float(charge)
    )

OPC_SITE_CHARGES_E = tuple(
    OPC_SITE_CHARGES_E
)

print()
print("=" * 78)
print("OPC WATER ELECTROSTATIC STORAGE AUDIT")
print("=" * 78)

print("Waters:", n_waters)
print("Ordering: O-H1-H2-M")
print(
    "M-site weights:",
    OPC_M_WEIGHTS,
)
...
print(
    "Stored sites/frame: O, H1, H2"
)
print(
    "M-site coordinates: exactly reconstructable"
)
print(
    "Water frame interval:",
    f"{WATER_TRAJECTORY_INTERVAL_PS:.1f} ps",
)
print(
    "Water frames:",
    WATER_N_FRAMES,
...
print()
print("=" * 78)
print("10 ns 300 K NVT PRODUCTION MD")
print("=" * 78)
...
print()
print("=" * 78)
print("INITIAL STATE FROM PRODUCTION RESCUE CHECKPOINT")
print("=" * 78)
...
screening_trajectory = np.load(
    SCREENING_TRAJECTORY,
    mmap_mode="r+",
)

if screening_trajectory.shape != (
    SCREENING_N_FRAMES,
    SCREENING_WIDTH,
):
    raise RuntimeError(
        "Existing screening trajectory shape mismatch: "
        f"{screening_trajectory.shape}"
    )

if screening_trajectory.dtype != np.float32:
    raise RuntimeError(
        "Existing screening trajectory dtype mismatch: "
        f"{screening_trajectory.dtype}"
    )
...
# Companion disk-backed OPC O/H/H trajectory.
#
# Shape:
#   [frame, water, site(O/H1/H2), xyz]
#
# 1001 frames for a 10 ns trajectory sampled every 10 ps.
WATER_TRAJECTORY.parent.mkdir(
    parents=True,
    exist_ok=True,
)

water_trajectory = np.load(
    WATER_TRAJECTORY,
    mmap_mode="r+",
)

if water_trajectory.shape != (
    WATER_N_FRAMES,
    n_waters,
    3,
    3,
    ...
    raise RuntimeError(
        "Existing water trajectory shape mismatch: "
        f"{water_trajectory.shape}"
    )

if water_trajectory.dtype != np.float32:
    raise RuntimeError(
        "Existing water trajectory dtype mismatch: "
        f"{water_trajectory.dtype}"
    )
...
# ------------------------------------------------------------
# Validate the final persisted screening frame against the exact
# rescue checkpoint before appending anything.
# ------------------------------------------------------------

last_screening_index = (
    screening_frames_written
    - 1
)

checkpoint_screening_frame = (
    pack_screening_frame(
        current_positions,
        RECOVERY_START_PRODUCTION_PS,
        checkpoint_time_ps,
...
persisted_screening_frame = np.asarray(
    screening_trajectory[
        last_screening_index,
        :
    ],
...
max_rescue_screening_difference = float(
    np.max(
        np.abs(
            persisted_screening_frame.astype(float)
            -
            checkpoint_screening_frame.astype(float)
        )
    )
)

if (
    max_rescue_screening_difference
    >
    2.0e-5
):

    raise RuntimeError(
        "Last persisted screening frame does not "
        "match the rescue checkpoint: "
        f"{max_rescue_screening_difference:.9e}"
    )


# ------------------------------------------------------------
# Validate the final persisted O/H/H water frame.
# ------------------------------------------------------------
...
persisted_water_frame = np.asarray(
    water_trajectory[
        last_water_index,
        :,
        :,
...
max_rescue_water_difference_nm = float(
    np.max(
        np.abs(
            persisted_water_frame.astype(float)
            -
            checkpoint_water_ohh.astype(float)
        )
    )
)

if (
    max_rescue_water_difference_nm
    >
    2.0e-5
):

    raise RuntimeError(
        "Last persisted water frame does not "
        "match the rescue checkpoint: "
        f"{max_rescue_water_difference_nm:.9e} nm"
    )


print()
print("=" * 78)
print("PERSISTED TRAJECTORY / RESCUE CHECKPOINT MATCH")
print("=" * 78)

print(
    "Last screening frame index:",
    last_screening_index,
)

print(
    "Last water frame index:",
    last_water_index,
)

print(
    "Max screening-frame difference:",
    f"{max_rescue_screening_difference:.3e}",
)

print(
    "Max water-frame difference:",
    f"{max_rescue_water_difference_nm:.3e} nm",
)

print(
    "4500 ps frame rewrite: NONE"
)

print(
    "Next screening frame index:",
    screening_frames_written,
)

print(
    "Next water frame index:",
    water_frames_written,
)
...
print()
print("=" * 78)
print("REPLICA B 4.5 -> 10 ns RECOVERY RUNTIME PREFLIGHT: PASS")
print("=" * 78)
print()
print(
    f"Recovery source production time: "
    f"{RECOVERY_START_PRODUCTION_PS:.3f} ps"
)
print(
    f"Production remaining: "
...
print(
    f"Persisted screening frames: "
    f"{screening_frames_written}"
)
print(
    f"Persisted water frames: "
    f"{water_frames_written}"
)
print(
    f"Screening/checkpoint max difference: "
    f"{max_rescue_screening_difference:.3e}"
)
print(
    f"Water/checkpoint max difference: "
    f"{max_rescue_water_difference_nm:.3e} nm"
)
print()
print("Production integration steps executed: 0")
print("New screening frames written: 0")
print("New water frames written: 0")
print("Boundary frame rewritten: NO")
print("Production checkpoints written: 0")
print("Checkpoint state reset: NONE")
print()
...
print(
    "OPC O/H/H electrostatic trajectory:"
)

print(
    f"  frames: {WATER_N_FRAMES}"
)

print(
    f"  waters/frame: {n_waters}"
)

print(
    "  sites/water stored: 3 (O, H1, H2)"
)

print(
    "  xyz components/site: 3"
)
...
# ============================================================
# 17. RECOVER INTERRUPTED PRODUCTION TO 10 ns AT 300 K
# ============================================================

print()
print("=" * 78)
print("RESUMING 300 K NVT PRODUCTION MD")
print("=" * 78)
...
# ============================================================
# 17B. FINALIZE COMPACT SCREENING + WATER TRAJECTORIES
# ============================================================
...
if (
    ...
    raise RuntimeError(
        "Incorrect number of water frames: "
        f"wrote {water_frames_written}, "
        f"expected {WATER_N_FRAMES}"
    )
...
screening_metadata = {
    ...
    "recovery_start_production_elapsed_ps":
        RECOVERY_START_PRODUCTION_PS,

    "persisted_frames_before_recovery":
        RESCUE_SCREENING_FRAMES,

    "recovery_boundary_frame_rewritten":
        False,
...
# ============================================================
# 17C. SAVE OPC O/H/H WATER TRAJECTORY METADATA
# ============================================================
...
water_metadata = {
    ...
    "opc_site_charges_e":
        {
            "O":
                OPC_SITE_CHARGES_E[0],
            "H1":
                OPC_SITE_CHARGES_E[1],
            "H2":
                OPC_SITE_CHARGES_E[2],
            "M":
                OPC_SITE_CHARGES_E[3],
        },

    "m_site_reconstruction":
        {
            "equation":
                "rM = wO*rO + wH1*rH1 + wH2*rH2",
            ...
            "exact_virtual_site_rule_from_system_xml":
                True,
        },

    "note":
        (
            "Only the three massive OPC sites O/H1/H2 are "
            "stored. The massless charged M site is exactly "
            "reconstructable from the serialized OpenMM "
            "ThreeParticleAverageSite rule recorded above."
        ),
    ...
    "companion_screening_trajectory":
        str(
            SCREENING_TRAJECTORY
        ),
...
if (
    ...
    status = "PRODUCTION_300K_10NS_PASS"
else:
    status = "PRODUCTION_300K_10NS_COMPLETE_INSPECT_WARNINGS"
...
metadata = {
    ...
    "stage":
        "10 ns 300 K NVT production",

    "interrupted_run_recovery":
        True,

    "recovery_source":
        "atomic 100 ps rescue checkpoint",

    "recovery_start_production_elapsed_ps":
        RECOVERY_START_PRODUCTION_PS,

    "recovery_remaining_production_ps":
        RECOVERY_REMAINING_PS,

    "recovery_source_openmm_time_ps":
        checkpoint_time_ps,

    "persisted_screening_frames_before_recovery":
        RESCUE_SCREENING_FRAMES,

    "persisted_water_frames_before_recovery":
        RESCUE_WATER_FRAMES,

    "recovery_screening_frame_max_difference":
        max_rescue_screening_difference,

    "recovery_water_frame_max_difference_nm":
        max_rescue_water_difference_nm,

    "recovery_rewrote_boundary_frame":
        False,

    "log_scope":
        (
            "Recovery checkpoint state plus resumed segment only; "
            "the original in-memory 0-4.5 ns thermodynamic log "
            "was not persisted before KeyboardInterrupt."
        ),
    ...
    "water_stored_sites_per_molecule":
        3,

    "water_m_site_reconstructable":
        True,
...
print()
print("=" * 78)
print("10 ns 300 K NVT PRODUCTION SUMMARY")
print("=" * 78)
...
print()
print("=" * 78)
print("10 ns 300 K NVT PRODUCTION: COMPLETE")
print("=" * 78)

print()
print(
    "This was unbiased 10 ns production MD following "
