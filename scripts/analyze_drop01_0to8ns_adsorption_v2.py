...
def rows_match_except_source(
    ...
    # Checkpoint-restored positions, velocities, temperature,
    # kinetic energy, and all geometry-derived observables must
    # reproduce tightly.
    #
    # Potential energy may be reevaluated with tiny numerical
    # differences when a CUDA mixed-precision Context is rebuilt.
    # Total energy inherits the same difference.
    energy_fields = {
        "potential_kJ_per_mol",
        "total_kJ_per_mol",
    }

    ENERGY_DUPLICATE_TOL_KJ_PER_MOL = 1.0e-2
    ...
    for key in fieldnames:
        ...
        if key in energy_fields:
            ...
            if diff > ENERGY_DUPLICATE_TOL_KJ_PER_MOL:
                return False

        else:

            if diff > VALUE_DUPLICATE_TOL:
                return False
