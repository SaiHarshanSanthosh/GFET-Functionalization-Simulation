...
# ============================================================
# DROP-01 CONTINUATION - EXACT CHECKPOINT 5 ns -> 8 ns
#
# INPUT:
...
# ============================================================
# 0. LOCKED REPLICA MANIFEST
# ============================================================

parser = argparse.ArgumentParser(
    description=(
        "Continue one locked drop-ensemble replica from its "
        "exact 5 ns checkpoint to 8 ns."
    )
)
...
if (
    len(DROP_ID) != 6
    or
    not DROP_ID.startswith("drop")
    or
    not DROP_ID[4:].isdigit()
):
    raise ValueError(
        "--drop-id must have exact form dropNN, "
        "for example drop02."
    )

if DROP_ID == "drop01":
    raise ValueError(
        "Drop01 is the validated reference trajectory. "
        "The generic ensemble runner is reserved for "
        "Drop02-Drop13."
    )
...
if MANIFEST_STATUS != "pending":
    raise RuntimeError(
        f"{DROP_ID} manifest status is "
        f"{MANIFEST_STATUS!r}, not 'pending'."
    )

START_HEIGHT_NM = float(
    MANIFEST_ROW["height_nm"]
)

REQUESTED_TILT_DEG = float(
    MANIFEST_ROW["tilt_deg"]
)

AZIMUTH_DEG = float(
    MANIFEST_ROW["azimuth_deg"]
)

SHIFT_A_FRAC = float(
    MANIFEST_ROW["shift_a_frac"]
)

SHIFT_B_FRAC = float(
    MANIFEST_ROW["shift_b_frac"]
)

VELOCITY_SEED = int(
    MANIFEST_ROW["velocity_seed"]
)

LANGEVIN_SEED = int(
    MANIFEST_ROW["langevin_seed"]
)

HORIZON_NS = float(
    MANIFEST_ROW["duration_ns"]
)
...
if not math.isclose(
    ...
    raise RuntimeError(
        "Locked drop ensemble requires an 8.0 ns "
        f"horizon; manifest has {HORIZON_NS}."
    )
...
if (
    ...
    raise RuntimeError(
        "Locked ensemble requires zero azimuth and "
        "zero lateral shifts."
    )
...
# ============================================================
# 2. REPLICA CONTINUATION PARAMETERS
# ============================================================
...
# VELOCITY_SEED and LANGEVIN_SEED come from the locked
# manifest. Exact dynamics are restored from the 5 ns
# OpenMM checkpoint below.
...
SYSTEM_XML = (
    Path("parameters/ensemble")
    /
    f"{DROP_ID}_fixedz0_wallfree_opc_yb.xml"
)

POSITIONS_FILE = (
    Path("parameters/ensemble")
    /
    f"{DROP_ID}_5ns_positions_nm.npy"
)

VELOCITIES_VALIDATION_FILE = (
    Path("parameters/ensemble")
    /
    f"{DROP_ID}_5ns_velocities_nm_per_ps.npy"
)

INPUT_CHECKPOINT = (
    Path("checkpoints/ensemble")
    /
    f"{DROP_ID}_5ns.chk"
)
...
OUTPUT_POSITIONS = (
    Path("parameters/ensemble")
    /
    f"{DROP_ID}_8ns_positions_nm.npy"
)

OUTPUT_VELOCITIES = (
    Path("parameters/ensemble")
    /
    f"{DROP_ID}_8ns_velocities_nm_per_ps.npy"
)
...
OUTPUT_LOG = (
    Path("analysis/ensemble")
    /
    f"{DROP_ID}_5ns_to_8ns_log.csv"
)
...
OUTPUT_DCD = (
    Path("trajectories/ensemble")
    /
    f"{DROP_ID}_graphene_ligand.dcd"
)

RESCUE_CHECKPOINT = (
    Path("checkpoints/ensemble")
    /
    f"{DROP_ID}_5ns_to_8ns_rescue.chk"
)
...
TRAJECTORY_REFERENCE_PDB = (
    Path("visualization/ensemble")
    /
    f"{DROP_ID}_initial.pdb"
)
...
# ============================================================
# 12-13. LOAD EXACT REPLICA 5 ns CHECKPOINT
#
# CRITICAL:
#   - no setPositions()
...
if abs(
    ...
    raise RuntimeError(
        "Checkpoint does not represent exactly 5000 ps: "
...
print()
print(f"{DROP_ID.upper()} LINKER STATE AT CONTINUATION START")
print("-----------------------------")
print(
...
# ============================================================
# 17. CONTINUE REPLICA: 5000 ps -> 8000 ps
# ============================================================

print()
print("=" * 78)
print(f"RUNNING {DROP_ID.upper()} CONTINUATION: 5000 -> 8000 ps")
print("=" * 78)
...
#
# Existing master trajectory contains:
#   t = 0, 1, ..., 5000 ps  => 5001 DCD models
#
# The first continuation frame must therefore be 5001 ps.
# ------------------------------------------------------------
...
with open(
    ...
    if (
        ...
        raise RuntimeError(
            f"Existing {DROP_ID.upper()} DCD has an invalid header."
        )
...
if existing_dcd_models != 5001:

    raise RuntimeError(
        "Expected exactly 5001 existing DCD models "
        "(0 through 5000 ps), found "
...
print(
    "First new trajectory frame will be: 5001 ps"
)
...
if (
    ...
    status = (
        f"{DROP_ID.upper()}_8NS_CONTINUATION_PASS"
    )
    ...
    status = (
        f"{DROP_ID.upper()}_8NS_CONTINUATION_INSPECT_WARNINGS"
    )
...
metadata = {
    ...
    "stage":
        f"{DROP_ID} unbiased exact-checkpoint continuation 5000-8000 ps",
    ...
    "predetermined_horizon_ns":
        HORIZON_NS,

    "continuation_start_time_ps":
        5000.0,

    "continuation_end_time_ps":
        8000.0,

    "checkpoint_loaded":
        True,

    "velocity_regeneration_on_continuation":
        False,

    "time_reset_on_continuation":
        False,
    ...
    "fresh_velocities_generated_in_this_continuation":
        False,
...
print()
print("=" * 78)
print(f"{DROP_ID.upper()} 8 ns CONTINUATION SUMMARY")
print("=" * 78)
...
print()
print("=" * 78)
print(f"{DROP_ID.upper()} 8 ns CONTINUATION: COMPLETE")
print("=" * 78)
