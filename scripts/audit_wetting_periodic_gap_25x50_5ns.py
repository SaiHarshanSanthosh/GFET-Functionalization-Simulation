...
OUTPUT_FIGURE = (
    ...
    / "23_graphene_opc_wetting_25x50_periodic_gap_500to5000ps.png"
)
...
print()
print("=" * 80)
print("GRAPHENE / OPC 5 ns PERIODIC DRY-GAP AUDIT")
print("=" * 80)
...
# ============================================================
# 500 ps BLOCKWISE PERIODIC-GAP STATISTICS
# ============================================================
...
print()
print("=" * 80)
print("500 ps BLOCKWISE PERIODIC-GAP STABILITY")
print("=" * 80)

print(
    "block_ps       frames   mean_width_nm   max_width_nm   "
    "mean_gap_nm   min_gap_nm"
)

print("-" * 80)


for i, (start_ps, end_ps) in enumerate(BLOCKS_PS):
    ...
    block_widths = occupied_widths[
        block_mask
    ]

    block_gaps = largest_gaps[
        block_mask
    ]
    ...
    row = {
        ...
        "mean_occupied_width_nm": float(
            np.mean(block_widths)
        ),
        "max_occupied_width_nm": float(
            np.max(block_widths)
        ),
        "mean_largest_dry_gap_nm": float(
            np.mean(block_gaps)
        ),
        "min_largest_dry_gap_nm": float(
            np.min(block_gaps)
        ),
    }
    ...
    print(
        ...
        f"      {len(block_times):3d}"
        f"        {row['mean_occupied_width_nm']:8.4f}"
        f"        {row['max_occupied_width_nm']:8.4f}"
        f"      {row['mean_largest_dry_gap_nm']:8.4f}"
        f"      {row['min_largest_dry_gap_nm']:8.4f}"
    )
...
ax.set_title(
    "Graphene-OPC Wetting 25x50: 500-5000 ps Periodic-Gap Audit"
)
