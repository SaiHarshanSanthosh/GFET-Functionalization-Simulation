...
#
# Face-on pyrene, 2.00 nm above equilibrated graphene.
#
# NO MD
# NO MINIMIZATION
# NO RESTRAINTS
...
parser = argparse.ArgumentParser(
    description=(
        "Prepare one unbiased pyrene-PEG5 drop initial condition "
        "from the validated FF-3 reference states."
    )
)
...
parser.add_argument("--shift-a-frac", type=float, default=0.0)
parser.add_argument("--shift-b-frac", type=float, default=0.0)

args = parser.parse_args()

DROP_ID = args.drop_id.lower().strip()

if (
    not DROP_ID.startswith("drop")
    or not DROP_ID[4:].isdigit()
):
    raise ValueError(
        "--drop-id must have form dropNN, e.g. drop04."
    )

TARGET_PYRENE_HEIGHT_NM = float(args.height_nm)
TARGET_TILT_DEG = float(args.tilt_deg)
TARGET_AZIMUTH_DEG = float(args.azimuth_deg)

SHIFT_A_FRAC = float(args.shift_a_frac)
SHIFT_B_FRAC = float(args.shift_b_frac)

if TARGET_PYRENE_HEIGHT_NM <= 0.0:
    raise ValueError("--height-nm must be positive.")

if not 0.0 <= TARGET_TILT_DEG <= 90.0:
    raise ValueError("--tilt-deg must lie in [0, 90].")

#
# The first locked ensemble deliberately keeps lateral registry
# fixed.  Reject rather than silently ignore any nonzero shift.
#
if (
    abs(SHIFT_A_FRAC) > 1.0e-15
    or abs(SHIFT_B_FRAC) > 1.0e-15
):
    raise ValueError(
        "This locked ensemble requires zero lateral shifts."
    )
...
OUTPUT_SOLUTE = (
    ...
    /
    f"{DROP_ID}_supported_solute_positions_nm.npy"
)

OUTPUT_WATER = (
    Path("parameters/ensemble")
    /
    f"{DROP_ID}_opc_water_positions_nm.npy"
)

OUTPUT_METADATA = (
    Path("analysis/ensemble")
    /
    f"{DROP_ID}_initialization.json"
)
...
print()
print("=" * 78)
print(f"{DROP_ID.upper()} INITIAL-CONDITION BUILD")
print("=" * 78)
print()
print(
...
# exactly 2.00 nm above the fitted graphene plane.
# ============================================================
...
# ============================================================
# 4A. OPTIONAL RIGID-BODY ORIENTATION
#
# Manifest tilt 0 means the validated reference face-on
# orientation is preserved exactly.  No rotation is performed.
#
# For nonzero target tilts, rotate the entire 70-atom ligand
# rigidly about the pyrene center.  No internal coordinate is
# modified.
# ============================================================

reference_pyrene_center = (
    ligand_drop[
        aromatic
    ].mean(
        axis=0
...
_, _, reference_pvh = np.linalg.svd(
    reference_pyrene_centered,
    full_matrices=False,
)

reference_pyrene_normal = (
    reference_pvh[-1].copy()
)

reference_pyrene_normal /= np.linalg.norm(
    reference_pyrene_normal
)
...
#
# e1 = laboratory +x projected into the fitted graphene plane.
# e2 completes a right-handed basis with the graphene normal.
...
if TARGET_TILT_DEG > 1.0e-12:
    ...
    target_pyrene_normal /= np.linalg.norm(
        target_pyrene_normal
    )
...
if (
    ...
    raise RuntimeError(
        "Requested pyrene tilt was not reproduced: "
        f"target={TARGET_TILT_DEG:.8f} deg, "
        f"actual={drop_tilt_deg:.8f} deg."
    )
...
metadata = {
    ...
    "status":
        f"{DROP_ID.upper()}_INITIAL_CONFIGURATION_PASS",
    ...
    "requested_azimuth_deg":
        TARGET_AZIMUTH_DEG,

    "reference_pyrene_tilt_deg":
        reference_tilt_deg,

    "rigid_rotation_applied_deg":
...
print()
print("=" * 78)
print(f"{DROP_ID.upper()} INITIAL CONFIGURATION: PASS")
print("=" * 78)
print(
