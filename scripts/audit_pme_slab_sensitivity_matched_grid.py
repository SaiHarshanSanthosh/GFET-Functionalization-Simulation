...
OUTPUT_JSON = (
    ROOT
    / "analysis"
    / "pme_slab_sensitivity_matched_grid_5500ps.json"
)
...
# ============================================================
# MATCHED PME PARAMETERS
#
# Keep alpha, nx, ny fixed.
#
# Choose nz = 8 * Lz so every test has exactly the
# same z-grid density: 8 PME grid points per nm.
#
# This separates box-height sensitivity from changes in
# reciprocal-space grid density.
# ============================================================

MATCHED_ALPHA = 2.62826088
MATCHED_NX = 50
MATCHED_NY = 45
...
for lz_nm in LZ_VALUES_NM:
    ...
    # --------------------------------------------------------
    # FORCE MATCHED PME RESOLUTION
    #
    # Same alpha, nx, ny for all boxes.
    # nz scales exactly with Lz.
    # --------------------------------------------------------

    requested_nz = MATCHED_NZ[
        float(lz_nm)
    ]
    ...
    print(
        f"  PME requested   = "
        f"alpha={MATCHED_ALPHA:.8f}  "
        f"grid={MATCHED_NX}x{MATCHED_NY}x{requested_nz}"
    )
