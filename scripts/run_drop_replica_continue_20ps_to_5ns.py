...
# ============================================================
# DROP-ENSEMBLE CONTINUATION - EXACT CHECKPOINT 20 ps -> 5 ns
#
# INPUT:
#   Exact 20 ps Stage-A checkpoint for the selected replica.
#   Positions and velocities are also loaded only for validation
#   against the checkpoint.
#
# THIS STAGE:
#   Unbiased NVT continuation
#   300 K
#   4980 ps continuation: absolute time 20 -> 5000 ps
#   1 fs timestep
...
#   - No velocity regeneration or time reset occurs.
...
parser = argparse.ArgumentParser(
    description=(
        ...
        "exact 20 ps checkpoint to 5 ns."
    )
)
...
# manifest. The exact continuation state itself is restored
# from the OpenMM checkpoint below.
...
POSITIONS_FILE = (
    Path("parameters/ensemble")
    /
    f"{DROP_ID}_stageA_20ps_positions_nm.npy"
)

VELOCITIES_VALIDATION_FILE = (
    Path("parameters/ensemble")
    /
    f"{DROP_ID}_stageA_20ps_velocities_nm_per_ps.npy"
)

INPUT_CHECKPOINT = (
    Path("checkpoints/ensemble")
    /
    f"{DROP_ID}_stageA_20ps.chk"
)
...
OUTPUT_LOG = (
    ...
    /
    f"{DROP_ID}_20ps_to_5ns_log.csv"
)
...
RESCUE_CHECKPOINT = (
    Path("checkpoints/ensemble")
    /
    f"{DROP_ID}_20ps_to_5ns_rescue.chk"
)
...
# ============================================================
# 12-13. LOAD EXACT REPLICA STAGE-A CHECKPOINT
#
# CRITICAL:
#   - no setPositions()
...
# ============================================================
# 17. CONTINUE REPLICA: 20 ps -> 5000 ps
# ============================================================

print()
print("=" * 78)
print(f"RUNNING {DROP_ID.upper()} CONTINUATION: 20 -> 5000 ps")
print("=" * 78)
...
if (
    ...
    status = (
        f"{DROP_ID.upper()}_5NS_CONTINUATION_PASS"
    )
    ...
    status = (
        f"{DROP_ID.upper()}_5NS_CONTINUATION_INSPECT_WARNINGS"
    )
...
metadata = {
    ...
    "stage":
        f"{DROP_ID} unbiased exact-checkpoint continuation 20-5000 ps",
...
print()
print("=" * 78)
print(f"{DROP_ID.upper()} 5 ns CONTINUATION SUMMARY")
print("=" * 78)
...
print()
print("=" * 78)
print(f"{DROP_ID.upper()} 5 ns CONTINUATION: COMPLETE")
print("=" * 78)
