...
# ============================================================
# STAGED NVT HEATING - STAGE 3: 150 K
#
# INPUT:
#   Completed FF-3 100 K wall-free/YB state.
#   Positions and velocities are continued directly.
#
# THIS STAGE:
#   NVT
#   150 K
...
PRIOR_STAGE_METADATA = Path(
    "analysis/"
    "ff3_pbs_surrogate_opc_yb_heating_100K.json"
)
...
OUTPUT_POSITIONS = Path(
    "parameters/combined/"
    "ff3_pbs_surrogate_opc_yb_"
    "heated_150K_positions_nm.npy"
)

OUTPUT_VELOCITIES = Path(
    "parameters/combined/"
    "ff3_pbs_surrogate_opc_yb_"
    "heated_150K_velocities_nm_per_ps.npy"
)

OUTPUT_CHECKPOINT = Path(
    "checkpoints/"
    "ff3_pbs_surrogate_opc_yb_heating_150K.chk"
)

OUTPUT_LOG = Path(
    "analysis/"
    "ff3_pbs_surrogate_opc_yb_heating_150K_log.csv"
)

OUTPUT_METADATA = Path(
    "analysis/"
    "ff3_pbs_surrogate_opc_yb_heating_150K.json"
)
...
if prior_stage_metadata.get(
    ...
    raise RuntimeError(
        "150 K heating requires a successful "
        "HEATING_100K_PASS input stage. "
...
if velocities_nm_per_ps.shape != (
    ...
    raise RuntimeError(
        ...
        f"velocities={velocities_nm_per_ps.shape}, "
        f"particles={n_particles}"
    )
...
if not np.all(
    ...
    raise RuntimeError(
        "Non-finite continuation velocity detected."
    )
...
if metadata_graphene_particles != N_GRAPHENE_TOTAL:
    raise RuntimeError(
        "Graphene count disagrees with assembly metadata."
    )

if metadata_ligand_particles != N_LIGAND:
    raise RuntimeError(
        "Ligand count disagrees with assembly metadata."
    )

if metadata_sites_per_water != SITES_PER_WATER:
    raise RuntimeError(
        "OPC sites-per-water disagrees with metadata."
    )

if n_ions != n_na + n_cl:
    raise RuntimeError(
        "Ion-count metadata is internally inconsistent."
    )
...
if na_end_index - na_start_index != n_na:
    raise RuntimeError(
        "Na+ range/count mismatch."
    )

if cl_start_index != na_end_index:
    raise RuntimeError(
        "Na+/Cl- boundary mismatch."
    )

if cl_end_index - cl_start_index != n_cl:
    raise RuntimeError(
        "Cl- range/count mismatch."
    )

if cl_end_index != metadata_total_particles:
    raise RuntimeError(
        "Final particle endpoint disagrees with metadata."
    )

if n_particles != metadata_total_particles:
    raise RuntimeError(
        "System particle count disagrees with metadata. "
...
if not bool(
    assembly_metadata["yeh_berkowitz_enabled"]
):
    raise RuntimeError(
...
# ============================================================
# 10. CREATE 150 K LANGEVIN INTEGRATOR
# ============================================================
...
# ============================================================
# 12. LOAD 100 K CONTINUATION POSITIONS
# ============================================================
...
# ============================================================
# 13. CONTINUE SAVED 100 K VELOCITIES
#
...
# 100 K occupied 20--40 ps.
# Continue the cumulative timeline from 40 ps.
context.setTime(
    40.0
    * unit.picoseconds
)
...
print()
print("=" * 78)
print("STAGED NVT HEATING - STAGE 3")
print("=" * 78)
...
print()
print("=" * 78)
print("INITIAL STATE ENTERING 150 K STAGE")
print("=" * 78)
...
# ============================================================
# 17. RUN 20 ps AT 150 K
# ============================================================

print()
print("=" * 78)
print("RUNNING 150 K NVT")
print("=" * 78)
...
if (
    ...
    status = (
        "HEATING_150K_COMPLETE_INSPECT_WARNINGS"
    )
...
print()
print("=" * 78)
print("150 K NVT HEATING: COMPLETE")
print("=" * 78)
...
print(
    "Do not start 200 K until this output is inspected."
)
