...
OUTPUT_CHECKPOINT = Path(
    "checkpoints/"
    "ff3_explicit_dpbs_opc_yb_production_300K_10ns.chk"
)

OUTPUT_LOG = Path(
    "analysis/"
    "ff3_explicit_dpbs_opc_yb_production_300K_5to10ns_log.csv"
)

OUTPUT_METADATA = Path(
    "analysis/"
    "ff3_explicit_dpbs_opc_yb_production_300K_10ns.json"
)

SCREENING_TRAJECTORY = Path(
    "analysis/"
    "ff3_explicit_dpbs_opc_yb_production_300K_5to10ns_"
...
RESCUE_CHECKPOINT = Path(
    "checkpoints/"
    "ff3_explicit_dpbs_opc_yb_production_300K_5to10ns_rescue.chk"
)
...
if prior_stage_metadata.get(
    ...
    raise RuntimeError(
        ...
        "PRODUCTION_300K_5NS_PASS input stage. "
...
# ============================================================
# SCREENING INSTRUMENTATION ZERO-STEP PREFLIGHT
# ============================================================

screening_test_row = pack_screening_frame(
    current_positions,
    0.0,
    initial_metrics["time_ps"],
)

if screening_test_row.shape != (
    SCREENING_WIDTH,
):
    raise RuntimeError(
        "Packed screening-frame shape mismatch: "
        f"{screening_test_row.shape}"
    )

if screening_test_row.dtype != np.float32:
    raise RuntimeError(
        "Packed screening frame is not float32."
    )
...
if not np.all(
    ...
    raise RuntimeError(
        "Packed screening frame contains non-finite values."
    )
...
print()
print("=" * 78)
print("SCREENING INSTRUMENTATION ZERO-STEP PREFLIGHT: PASS")
print("=" * 78)
...
print(
    "Screening file opened: NO"
)

print(
    "Rescue checkpoint written: NO"
)

print(
    "Production endpoint files written: NO"
)

print()

print(
    f"Expected screening frames: "
    f"{SCREENING_N_FRAMES}"
)

print(
    f"Expected screening columns/frame: "
    f"{SCREENING_WIDTH}"
)

print(
    f"Test packed row shape: "
    f"{screening_test_row.shape}"
)

print(
    f"Test packed row dtype: "
    f"{screening_test_row.dtype}"
)

print(
    f"Expected compact trajectory size: "
    f"{expected_bytes / 1024**2:.2f} MiB"
)
...
print(
    "Screening coordinate packing: PASS"
)

print(
    "Ready for actual 5-to-10 ns continuation launch."
)
